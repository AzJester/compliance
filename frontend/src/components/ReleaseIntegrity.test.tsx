...
describe('release integrity workflows', () => {
  it('requires an explicit reviewer and reason when waiving an incomplete CDRL', async () => {
    ...
    await user.click(screen.getByRole('button', { name: /save CDRL review/i }))

    expect(await screen.findByRole('status')).toHaveTextContent('CDRL readiness review saved')
  ...
  it('creates and displays an action linked to a requirement', async () => {
    const readiness: ReadinessSummary = {
      project_id: 'project-1', ready: false, readiness_percent: 80,
